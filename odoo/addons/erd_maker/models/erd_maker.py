...
class ErdMaker(models.TransientModel):
    ...
    def erd_transverse_html(self, trans_model):
        ...
        for field in self.env['ir.model.fields'].search([('model_id.model', '=', trans_model)]):
            ...
            if self.omit_builtin_fields == True and (
                    field.name == "id" or field.name == "display_name" or field.name == "create_date" or field.name == "create_uid" or field.name == "__last_update" or field.name == "write_date" or field.name == "write_uid"):
                continue
            field_name = field.name
            if (
                    field.ttype == "many2one" or field.ttype == "one2many") and self.current_transverse <= self.transverse_depth:
